	...
	bool Comandos::procesar(std::string &comando, MiRobot * robot ){
		...
		if(partes.size()==0){
			...
			mostrar=true;
				...
				case 'p':
					//p union tipo valor
					//Parametriza la uniÃ³n con el valor indicado V=Velocidad constante, VM=con PID
					if(partes.size()>3){
				...
				case 'm':
					...
					//Mueve la uniÃ³n hasta la posiciÃ³n en radianes indicada
					if(partes.size()>2){
				...
				case 'e':
					...
					{
					if( ruta==""){
						//AÃºn no tenemos una ruta. La cogemos del fichero
						...
						//Como tenemos una ruta se la aÃ±adimos al contenido
				...
				case 'w':
					//w [union]
					//Espera hasta que la uniÃ³n halla llegado a su destino. Si no se indica la uniÃ³n se espera hata terminar todas
					ROS_INFO("esperando");
